...
const std::string ServerSession::HTTP_METHODS[]={
...
void ServerSession::in_recv(const string &data) {
    if (status == HANDSHAKE) {
        ...
        string query_addr = isTrojanReq ? req.address.address : config.remote_addr;
        string query_port = to_string(isTrojanReq ? req.address.port : config.remote_port);
        if (isTrojanReq) {
