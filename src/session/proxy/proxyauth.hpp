...
class ProxyDelegate:public std::enable_shared_from_this<ProxyDelegate>{
    protected:
        ...
        const Config& config;
        virtual void async_auth(std::function<void (const system::error_code error)> resultHandler) =0;
    public:
        ...
        virtual void async_finished(std::function<void (asio::ip::tcp::socket in_socket,asio::ip::tcp::socket out_socket, const system::error_code error)> completeHandler) final {
            this->async_auth([this,completeHandler](const system::error_code error){
